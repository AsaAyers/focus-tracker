...
import { LOGFILE, FocussedWindow, Record, UsageCallback } from '../constants'
...
function parseLine(line: string): FocussedWindow {
  let record
  try {
    record = JSON.parse(line)
  } catch (e) {
  }
  return { ts: 0, app: "", title: "", ...record }
}

function reducer(data: Data, parsedLine: FocussedWindow) {
  const { last } = data
...
export default function gatherUsage(date: Date, callback: UsageCallback) {
  const id = uuid()
  ...
  let sentData: Data = { ...data, records: [] }
  function sendReport() {
    const line = {
  ...
  tail.on('error', (err: Error) => { throw err })
